...
from isl.utils.circuit_operations import QASM_SIM, SV_SIM, MPS_SIM, CUQUANTUM_SIM_EXPERIMENTAL, \
    CUQUANTUM_SIM
    ...
    def test_given_cuquantum_not_installed_when_cuquantum_backend_called_then_error(self):
        ...
        try:
            ...
            with self.assertRaises(ModuleNotFoundError):
                ISLRecompiler(qc, backend=CUQUANTUM_SIM_EXPERIMENTAL)
...
class TestISLCuquantum(TestCase):
    ...
    def test_given_cuquantum_backend_when_get_entanglement_then_correct_mps_from_circuit_called(self, mock_cuquantum_mps_from_circuit):
        ...
        recompiler = ISLRecompiler(qc, backend=CUQUANTUM_SIM_EXPERIMENTAL)
        recompiler._get_all_qubit_pair_entanglement_measures()
        mock_cuquantum_mps_from_circuit.assert_called_once()

    def test_given_cuquantum_experimental_backend_when_recompile_then_happy_path(self):
        logging.basicConfig()
        logging.getLogger('isl').setLevel(logging.DEBUG)
    ...
    def test_given_random_state_when_cuquantum_or_statevector_then_local_cost_equal(self):
        qc = co.create_random_initial_state_circuit(4)

        sv_recompiler = ISLRecompiler(qc, backend=SV_SIM)
        cuquantum_recompiler = ISLRecompiler(qc, backend=CUQUANTUM_SIM)

        self.assertAlmostEqual(
            sv_recompiler._evaluate_local_cost_sv(),
            cuquantum_recompiler._evaluate_local_cost_mps())

    def test_given_random_state_when_cuquantum_or_statevector_then_global_cost_equal(self):
        qc = co.create_random_initial_state_circuit(4)
        ...
        self.assertAlmostEqual(
            sv_recompiler._evaluate_global_cost_sv(),
            cuquantum_recompiler._evaluate_global_cost_mps())